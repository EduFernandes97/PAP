...
if ($dados ['ano'] == "12"){
    ?>
    <div id="botao">
        <input type="button" class="pure-button " value="11º Ano" onclick="popup_visible('av.php')">
    </div>
<?php
}
...
$sql = "SELECT * FROM av_aluno_total \n"
    . "WHERE num_processo = " . $_SESSION['num_proc'] ." and cod_anoLetivo = " . $_SESSION['ano'];
$sql_bd = mysqli_query($ligaBD, $sql);
...
$sql = "SELECT count(*) as num FROM `aluno` \n"
    ...
    . "WHERE aluno.cod_anoLetivo = " . $_SESSION['ano'] . " and aluno.num_processo = " . $_SESSION['num_proc'] . " and objetivo_aluno.dominio = \"Cognitivo\" and objetivo_aluno.cod_anoLetivo =" . $_SESSION['ano'];
...
$sql = "SELECT count(*) as num FROM aluno \n"
    . "	JOIN objetivo_aluno on objetivo_aluno.cod_curso = aluno.cod_curso\n"
    . "WHERE aluno.cod_anoLetivo = " . $_SESSION['ano'] . " and aluno.num_processo = " . $_SESSION['num_proc'] . " and objetivo_aluno.dominio = \"Socioafetivo\" and objetivo_aluno.cod_anoLetivo =" . $_SESSION['ano'] ;
...
$sql = "SELECT * FROM aluno \n"
    . "	JOIN objetivo_aluno on objetivo_aluno.cod_curso = aluno.cod_curso\n"
    . "WHERE aluno.cod_anoLetivo = " . $_SESSION['ano'] . " and aluno.num_processo = " . $_SESSION['num_proc'] ." and objetivo_aluno.cod_anoLetivo =" . $_SESSION['ano']." and objetivo_aluno.cod_anoLetivo = " . $_SESSION['ano']." order by dominio";
...
$sql = "SELECT av_aluno.nota, av_aluno.num_objetivo_aluno, objetivo_aluno.dominio, objetivo_aluno.objetivo FROM aluno
  ...
  WHERE aluno.num_processo = " . $_SESSION['num_proc'] . "  and aluno.cod_anoLetivo =" . $_SESSION['ano'] ." and av_aluno.cod_anoLetivo =" . $_SESSION['ano']." and objetivo_aluno.cod_anoLetivo = " . $_SESSION['ano']." order by dominio";
echo $sql;
...
<form onsubmit="atualiza_av();return false;" class="pure-form">
    <table border=1 width=900>
        ...
        <tr>
            ...
            <td align="center"><input max='5' min'1'name='nota[<?php echo $notas["num_objetivo_aluno"] ?>]' type='number' max='5' min='1' size='5' value='<?php echo $notas["nota"] ?>'></td>
        </tr>
...
for ($i = 1; $i < $num_cognitivo ; $i++) {
    ...
    echo "
			<tr>
				...
				<td align=\"center\"><input name='nota[" . $notas["num_objetivo_aluno"] . "]' type='number' max='5' min='1' size='5' value='" . $notas["nota"] . "'></td>
			</tr>
		";
...
<tr>
    ...
    <td align="center"><input name='nota[<?php echo $notas["num_objetivo_aluno"] ?>]' type='number' max='5' min='1' size='5'
...
for ($i = $num_cognitivo; $i < $num_objetivo - 1; $i++) {
    ...
    echo "
			<tr>
				<td >" . $objetivos["objetivo"] . "</td>
				<td align=\"center\"><input name='nota[" . $notas["num_objetivo_aluno"] . "]' max='5' min='1' type='number' size='5' value='" . $notas["nota"] . "'></td>
			</tr>
		";
...
<textarea maxlength='500' name='obs_aluno' style='height:135px;width:900px;''>".$dados_total['observacao']."</textarea>";
...
<table border=1 width=900>
    ...
    <td rowspan='<?php echo $num_cognitivo ?>' align="center"><b>Cognitivo</b></td>
    <td><?php echo $objetivos["objetivo"] ?></td>
    <td align="center"><input max="5" min="1" name='nota[<?php echo $objetivos["num_objetivo_aluno"] ?>]' type='number' size='5'></td>
</tr>
...
for ($i = 1; $i < $num_cognitivo ; $i++) {
    ...
    echo "
			<tr>
				<td >" . $objetivos["objetivo"] . "</td>
				<td align=\"center\"><input  max='5' min='1' name='nota[". $objetivos["num_objetivo_aluno"] ."]' type='number' size='5' ></td>
			</tr>
		";
...
for ($i = $num_cognitivo; $i < $num_objetivo -1; $i++) {
    ...
    echo "
			<tr>
				<td >" . $objetivos["objetivo"] . "</td>
				<td align=\"center\"><input   max='5' min='1' name='nota[". $objetivos["num_objetivo_aluno"] ."]' type='number' value='" . $notas["nota"] . "'></td>
			</tr>
		";
...
<br>
Observações do aluno formando (Autoavaliação Global):<br>
<textarea maxlength='500' name='obs_aluno' style='height:135px;width:900px;''></textarea>";
if ($num_objetivo == 0){
    echo "<br><b>O professor orientador ainda não defeniu os objetivos.</b>";

}
else
    echo "<br><br><input type='submit' class=\"pure-button \"></form>";
